...
class CargosManager(): 
    ...
    def imprimir_cargos(self, cargos):
    ...
    ##VER
    def get_cargos_str(self, cargos):
    ...
    #--------------------------------------------
    def filtrar_x_campo(self, cargos, campo, valores):
        filtrado = []
    ...
    ##VER
    def preparar_correo(self, cargos_a_enviar, url, filtros_str):
    ...
    def procesar_fuente(self, url_fuente):#, url, filtros):
        
        raw_html = self.simple_get(url_fuente)
        #raw_html = self.imprimir_cargossimple_get(url_fuente)
        #print(url_fuente)
        ...
        for x in range(1, cant_paginas+1): 
            ...
            cargos = self.extraerCargos(html)
            cargos_all += cargos
    ...
    def get_llamados_vigentes(self, fuente):
        ...
        #Fecha fija        
        today = datetime.strptime('30-05-2022', '%d-%m-%Y')
    ...
    def get_llamado_reciente(self, fuente):
    ...
    # GUARDA el resultado en cargos_todos
    def procesar_llamado_guardar(self, url_tipo_llamado):

        fuente = url_tipo_llamado + '?tipo=1'

        # procesa SOLO el llamado más reciente de la lista
        id_llamado = self.get_llamado_reciente(url_tipo_llamado)['id']
        
        #POC para todos los llamados
        llamados_vigentes = self.get_llamados_vigentes(url_tipo_llamado)
        ...
        for llamado_vigente in llamados_vigentes:
            ...
            for nivel in niveles:
                ...
                for distrito in distritos:
                    ...
                    #continue
                    cargos_all = self.procesar_fuente(fuente_llamado_distrito_nivel)


                    # Filtro nombre cargos
                    cargos_filtrados = self.filtrar_x_campo(cargos_all, 'asignatura', valores_filtro)

                    if len(cargos_filtrados) > 0:
                        print(f' \t {len(cargos_filtrados)}')
                        self.imprimir_cargos(cargos_filtrados)
                        #print(f'Cargos filtrados: {len(cargos_filtrados)}')

                    if len(cargos_all) > 0:
                        print("append " + str(len(cargos_all)))
                        #self.cargos_todos.append(cargos_all)
                        self.cargos_todos += cargos_all
...
#                        print(fuente_llamado_distrito_nivel)
    ...
    def procesar_llamado(self, url_tipo_llamado):
    ...
    def __init__(self):
        ...
        self.tipos_llamados = ['https://coberturas.neuquen.edu.ar/asambleas/frontend/web/index.php/vacante/listado',
...
#cargos = Cargosmager()
        ...
        self.tipos_llamados_vigentes = []
        for tipo_llamado in self.tipos_llamados:
            vigentes = self.get_llamados_vigentes(tipo_llamado)
            
            if len(vigentes) > 0:
                print ("hay vigentes")
                self.tipos_llamados_vigentes.append(tipo_llamado)
...
# Procesa los tipos de llamados 
        for llamado in self.tipos_llamados_vigentes:
            print("")
            #self.procesar_llamado(llamado)
            self.procesar_llamado_guardar(llamado)

        for cargo in self.cargos_todos:
            print("")
            print(cargo)
